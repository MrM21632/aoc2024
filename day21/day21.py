...
def compute_complexity(robot_id: int, curr_key: str, dest_key: str) -> int:
    ...
    for perm in set(itertools.permutations(sequence)):
        ...
        for i, key in enumerate(perm):
            steps += compute_complexity(robot_id + 1, 'A' if i == 0 else perm[i - 1], key)
            d = DIRS[key]
            r, c = r + d[0], c + d[1]
            if (r, c) not in pad.values():
                break
        else:
            steps += compute_complexity(robot_id + 1, perm[-1], 'A')
            candidates.append(steps)
    return min(candidates)


def get_complexity_of_codes(input_file: str) -> int:
    codes = get_codes(input_file)

    total_complexity = 0
    for code in codes:
        complexity = compute_complexity(0, 'A', code[0])
        for i in range(1, len(code)):
            complexity += compute_complexity(0, code[i - 1], code[i])
...
if __name__ == "__main__":
    ...
    print('The test input result is ', get_complexity_of_codes('test_input.txt'))  # 126384
    print('The main input result is ', get_complexity_of_codes('input.txt'))

    print('\n\n===== DAY 20, PUZZLE 2 =====')
    # print('The test input result is ', get_complexity_of_codes('test_input.txt'))
    # print('The main input result is ', get_complexity_of_codes('input.txt'))
